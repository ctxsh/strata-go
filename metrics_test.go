...
func TestMetricsCounter(t *testing.T) {
	...
	assert.NoError(t, testutil.CollectAndCompare(elem.Get(name, prometheus.Labels(labels)), strings.NewReader(expected)))
...
func TestMetricsGauge(t *testing.T) {
	...
	assert.NoError(t, testutil.CollectAndCompare(elem.Get(name, prometheus.Labels(labels)), strings.NewReader(expected)), "name")
